 ...
 * @date   April, 2019
 *
 * This file shows a sample implementation for a class that wraps dynamic memory management through its constructors / destructor / remaining methods for the purposes of CS-202.
...
{ 
  if (m_size){  //allocate only when it makes sense
    m_data = new (std::nothrow) int [m_size];  //use non-throwing new
    if (m_data){
